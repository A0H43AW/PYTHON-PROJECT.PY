...
def GetValue(event):
    ...
    e4.insert(0,select['LECTURE NUMBER ATTENDED'])
...
def Add():
    ...
    mysqldb=mysql.connector.connect(host="localhost",user="root",password="",database="REGISTRY")
    mycursor=mysqldb.cursor()

    try:
       sql = "INSERT INTO  registation (ID,STUDENT NAME,mobile,LECTURE NUMBER ATTENDED) VALUES (%s, %s, %s, %s)"
...
def update():
    ...
    try:
       sql = "Update  registation set empname= %s,mobile= %s,LECTURE NUMBER ATTENDED= %s where ID= %s"
...
def show():
        mysqldb = mysql.connector.connect(host="localhost", user="root", password="", database="REGISTRY")
        mycursor = mysqldb.cursor()
        mycursor.execute("SELECT ID,STUDENT NAME,mobile,LECTUURE NUMBER ATTENDED FROM registation")
...
root.configure(bg="#336699")  # Set background color to a shade of blue
global e1
global e2
global e3
global e4
...
Label(root, text="LECTURE NUMBER ATTENDED").place(x=10, y=100)
...
cols = ('ID', 'STUDENT NAME', 'mobile','LECTURE NUMBER ATTENDED')
